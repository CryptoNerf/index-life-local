    ...
    {#if devices.length}
      <div class="dev-list">
        ...
        {#if devices.length === 1 && devices[0].self}
          {#if provider === 'google'}
            <p class="hint">
              ⚠ Компьютер не виден. Важно: папка Google Drive на компьютере
              (через программу Google Drive) этому приложению <b>не видна</b> —
              на компьютере откройте Синхронизация → режим
              «Google Drive (напрямую)» → «Войти в Google» тем же аккаунтом.
            </p>
          {:else}
