...
(например VS Code по умолчанию) плеер может не отобразиться —
...
>
> **Если вместо синего окна — «Интеллектуальное управление приложениями
> заблокировало приложение»** (без кнопки «Выполнить в любом случае») — это
> Smart App Control на новых установках Windows 11: он жёстко блокирует
> неподписанные программы, обойти его кнопкой нельзя. Единственный способ
> запустить: Параметры → Конфиденциальность и защита → Безопасность Windows →
> Управление приложениями/браузером → Параметры интеллектуального управления
> приложениями → **Выкл**. ⚠️ Учтите: включить Smart App Control обратно можно
> только переустановкой/сбросом Windows. Подробнее —
> [docs/BUILD_SIGNING.md](docs/BUILD_SIGNING.md).
...
**Сохранение авторства.** К проекту приложены дополнительные условия по
разделу 7 AGPL-3.0 — см. файл [COPYRIGHT](COPYRIGHT). В частности, при
любом распространении (включая форки и модифицированные версии) обязательно
сохраняется указание оригинального авторства Émile Alexanyan в README и в
видимых «правовых уведомлениях» работающей программы (например, на странице
«About» или в подобном месте). Модифицированные версии нельзя выдавать за
оригинальный index.life, а имя автора нельзя использовать в рекламных
целях. Раздел 7 AGPL прямо разрешает такие условия (категории b, c, d),
и они **не могут быть удалены** ни при каком дальнейшем распространении.
