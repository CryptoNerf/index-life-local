...
| `customization_intro.py` | `CustomizationIntro` | Russian | Four acts of customisation in turn: "index.life" appears in Times + brand cyan and each letter cycles through different fonts and colours with a cascade ("Кастомизируй шрифт"); a phone-shaped page mock-up cycles through six background presets ("Кастомизируй фон"); a 2×2 grid of chart pictograms (heatmap / spiral / rose / river) cycles its accent colours every ~1 s ("Кастомизируй графики"); finally all three sit side by side under the title "Кастомизация" (~23 s) |
